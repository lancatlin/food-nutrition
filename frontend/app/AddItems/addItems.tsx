...
const mockItems: FoodItem[] = [
  { id: 1,  name: "Chicken Breast",    emoji: "🍗", expiry: new Date(2026, 2, 18), checked: true  },
  { id: 2,  name: "Beef",              emoji: "🥩", expiry: null,                  checked: true  },
  { id: 3,  name: "Milk",              emoji: "🥛", expiry: new Date(2026, 2, 21), checked: true  },
  { id: 4,  name: "Eggs",              emoji: "🥚", expiry: new Date(2026, 2, 31), checked: true  },
  { id: 5,  name: "Pasta",             emoji: "🍝", expiry: null,                  checked: true  },
  { id: 6,  name: "Parmesan Cheese",   emoji: "🧀", expiry: null,                  checked: true  },
  { id: 7,  name: "Toast",             emoji: "🍞", expiry: new Date(2026, 2, 25), checked: true  },
  ...
  { id: 12, name: "Dishwashing Liquid",emoji: "🧴", expiry: null,                  checked: false },
];
...
export default function AddItems() {
  ...
  const setExpiry = (id: number, date: Date | null) =>
    setItems(prev => prev.map(item => item.id === id ? { ...item, expiry: date } : item));
    ...
    <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 flex flex-col px-6 pt-14 pb-28">
          ...
          <div className="bg-background rounded-3xl p-8 flex flex-col items-center gap-6">
            ...
            <p className="text-fg text-xl font-semibold text-center leading-snug">
              Select a File or<br />Open Camera
            </p>
            <div className="flex gap-8">
              <button onClick={() => setPageState("processing")} className="text-primary font-semibold text-sm hover:text-primary-dark transition-colors">
                Select File
              </button>
              <button onClick={() => setPageState("processing")} className="text-primary font-semibold text-sm hover:text-primary-dark transition-colors">
                Open Camera
            ...
            <ul className="divide-y divide-border">
              {items.map(item => (
                <li key={item.id} className="flex items-center gap-3 px-4 py-3">
                  ...
                  <span className={`flex-1 font-medium text-sm ${item.checked ? "text-fg-secondary" : "text-fg-muted"}`}>
                    {item.name}
                  </span>
                  <button
                  ...
                  >
                    {item.expiry && <span className="text-primary">{formatExpiry(item.expiry)}</span>}
                    <i className={`fa-regular fa-calendar text-base ${item.expiry ? "text-primary" : "text-fg-muted"}`} />
                  </button>
                  <div
                    onClick={() => toggleItem(item.id)}
                    className={`w-5 h-5 rounded border-2 flex items-center justify-center shrink-0 cursor-pointer transition-all ${
                      item.checked ? "bg-primary border-primary" : "border-border-strong hover:border-primary-light"
                    }`}
                  >
                    {item.checked && <i className="fa-solid fa-check text-white text-[10px]" />}
                  </div>
                </li>
              ))}
            </ul>
      ...
      {calendarFor !== null && calendarItem && (
        <Calendar
          ...
          onConfirm={date => { setExpiry(calendarFor, date); setCalendarFor(null); }}
